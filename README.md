...
## 🌐 100% 오프라인 Local-First AI 비서
Langester는 프라이버시와 오프라인 생산성을 최우선으로 설계되었습니다. 초기 모델 다운로드 이후에는 **인터넷 연결 없이도** 다음의 핵심 기능들을 완벽하게 사용할 수 있습니다.

*   **오프라인 AI 분석:** 내장 Gemini Nano를 사용하여 비행기 모드에서도 실시간 대화 요약 및 답변 추천이 가능합니다. 데이터가 외부 서버로 전송되지 않아 보안이 완벽합니다.
*   **로컬 저장소 전용:** 인터넷 연결 없이도 분석된 리포트를 마크다운(.md) 파일로 즉시 생성하여 내 PC에 저장할 수 있습니다.
*   **화면 텍스트 캡처:** 브라우저 내부 엔진만 사용하여 웹페이지나 자막 내용을 즉시 추출합니다.
